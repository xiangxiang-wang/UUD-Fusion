...
class CVC14(Dataset):
    ...
    def __getitem__(self, item):
        if self.rgb:
            ...
            img2 = Image.open(os.path.join(self.dataset_dir2, self.file_list1[item]))
        else:
            img1 = Image.open(os.path.join(self.dataset_dir1, self.file_list1[item])).convert('L')
            img2 = Image.open(os.path.join(self.dataset_dir2, self.file_list1[item])).convert('L')
